...
/**
 * Rate cards stack on scroll: the first card sticks and holds, then the
 * next one slides up from below and attaches just under it — each new
 * card's sticky offset is nudged down a bit from the previous one, so the
 * pile visibly grows (a sliver of every earlier card stays peeking out
 * from behind) instead of one card abruptly replacing the last. The
 * runway (the tall spacer each card sits inside) is what makes a card
 * pause before the next one arrives.
 */
function RateCards() {
  return (
    <section className="py-16 sm:py-20">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        {cards.map((c, i) => {
          ...
          return (
            <div key={c.rateType} className={isLast ? '' : 'h-[60vh] sm:h-[50vh]'}>
              <div className="sticky" style={{ top: `${96 + i * 20}px`, zIndex: i + 1 }}>
                <motion.div
                ...
                >
                  ...
                  <img
                    src={c.image}
                    alt=""
                    className="w-16 h-16 sm:w-28 sm:h-28 rounded-xl object-cover shrink-0"
                  />
                </motion.div>
              </div>
            </div>
